...
/*		if ($DebugMode == "on") {
		...
		$lastentry = array();
		$sqlData = array();
		$result = $psDB->query("SELECT * FROM schedule ORDER BY id ASC");
		$sqlSchedule = $result->fetchArray();
		foreach($sqlSchedule as $sqlData) {	
			$colspan = 1;
			$timeData .= "<td>" . $sqlData['id'] . "</td><td>" . $sqlData['dayOfWeek'] . "</td><td>" . $sqlData['startTime'] . "</td><td>" . $sqlData['section'] . "</td><td>" . $sqlData['title'] . "</td><td>" . $sqlData['strictTime'] . "</td><td>" . $sqlData['duration'] . "</td><td>" . $sqlData['timeShift'] . "</td><td>" . $sqlData['overlapMax'] . "</td><td>" . $sqlData['xtra'] . "</td>";
		}
	}
