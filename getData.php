...
foreach ($DBarray as $databasefile) { //do the following for each database file
	...
	if($databasefile) {
		...
		if($sqlNow) {	
			...
			if ($sqlData['notes'] != NULL) {
				$notes = str_replace('\'', '', $sqlData['notes']);
				$channelData .= "<div class='tooltip'><span class ='tooltiptext'>" . $notes . "</span>";
			}
